%% A_sigma_ba_N.m: 不同网络规模(N)下的BA网络滞后效应对比
clear; clc; close all;
...
% ==========================================
TOPO_TYPE = 'BA';
M_BA      = 3;          % 固定优先连接参数 (2m 约等于平均度，设为 3 以匹配 ER 的 k=6)
...
if RUN_SIMULATION
    ...
    for i = 1:num_n
        ...
        fprintf('[SIM] 现正扫描: N=%d, m=%d ...\n', dyna_local.N, M_BA);
        % BA 网络接口：TOPO_PARAM 传递 m 参数
...
for i = 1:num_n
    dyna_local = DYNA_list{i};
    % BA网络的文件名中，TOPO_PARAM (m) 以 p%.3f 格式保存
