%% A_sigma_ws_K.m: 不同网络层数(K)下的WS网络滞后效应对比
clear; clc; close all;
...
% ==========================================
if RUN_SIMULATION
    fprintf('\n[SIM] 正在尝试启动 %d 并行仿真任务 (WS Network Layers)... \n', num_k);
    simStart = tic;
    for i = 1:num_k
        dyna_local = DYNA_list{i};
        fprintf('[SIM] 现正扫描: K=%d, pr=%.2f ...\n', dyna_local.K, P_REWIRE);
...
if isempty(Results) || (num_k > 0 && isempty(Results{1}))
    ...
    % title(sprintf('WS Network Hysteresis Loop vs Layers (N=%d, K_{avg}=%d)', DYNA.N, K_AVG));

    % 图像导出
    plots_dir = fullfile(fileparts(fileparts(mfilename('fullpath'))), 'manuscript', 'V2', 'manuscirpt', 'figures');
    if ~exist(plots_dir, 'dir'), mkdir(plots_dir); end
    % fname = fullfile(plots_dir, 'fig5b.eps');
    ...
    test_fname = fullfile(test_plots_dir, 'fig5b.png');
