%% A_sigma_ba_K.m: 不同网络层数(K)下的BA网络滞后效应对比
clear; clc; close all;
...
% ==========================================
TOPO_TYPE = 'BA';
M_BA      = 3;          % 固定优先连接参数 (设为 3 以匹配 ER 的 k=6)
...
% ==========================================
if RUN_SIMULATION
    fprintf('\n[SIM] 正在尝试启动 %d 并行仿真任务 (BA Network Layers)... \n', num_k);
    simStart = tic;
    for i = 1:num_k
        dyna_local = DYNA_list{i};
        fprintf('[SIM] 现正扫描: K=%d, m=%d ...\n', dyna_local.K, M_BA);
...
for i = 1:num_k
    ...
    mat_name = sprintf('hysteresis_%s_N%d_K%d_p%.3f_a%.3f_b%.3f_n%.2f_results.mat', ...
        lower(TOPO_TYPE), dyna_local.N, dyna_local.K, M_BA, dyna_local.alpha, dyna_local.beta, dyna_local.noise);
...
if isempty(Results) || (num_k > 0 && isempty(Results{1}))
    ...
    % title(sprintf('BA Network Hysteresis Loop vs Layers (N=%d, m=%d)', DYNA.N, M_BA));

    % 图像导出
    plots_dir = fullfile(fileparts(fileparts(mfilename('fullpath'))), 'manuscript', 'V2', 'manuscirpt', 'figures');
    if ~exist(plots_dir, 'dir'), mkdir(plots_dir); end
    % fname = fullfile(plots_dir, 'fig5c.eps');
    ...
    test_fname = fullfile(test_plots_dir, 'fig5c.png');
